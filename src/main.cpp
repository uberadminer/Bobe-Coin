...
uint256 hashGenesisBlock("0x148082a0538cd942c16b4a26a2eb9bb59eb174632bd3bd33dadf00e4810be293");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BobeCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BobeCoin Signed Message:\n";
...
{
    // BobeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BobeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BobeCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BobeCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x148082a0538cd942c16b4a26a2eb9bb59eb174632bd3bd33dadf00e4810be293");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1393868618;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084922160;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BobeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BobeCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running BobeCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BobeCoinMiner terminated\n");
        throw;
    }
}
